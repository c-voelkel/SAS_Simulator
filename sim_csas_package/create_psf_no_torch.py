...
from sim_csas_package.utils_no_torch import save_sas_plot, c2g
...
class CreatePSF:
    def __init__(self, sys_config, save_img_dir, save_data_dir):
        ...
        self.dev = 'cpu'  # You can change this to 'cuda:0' if you have a GPU.
    ...
    def run(self):
        with np.warnings.catch_warnings():
            ...
            single_scatterer = np.zeros((pix_dim, pix_dim))
            phase = np.zeros((pix_dim, pix_dim))
            single_scatterer[int(pix_dim // 2), int(pix_dim // 2)] = 1

            single_scatterer = single_scatterer.ravel()[RP.circle_indeces]
            phase = phase.ravel()[RP.circle_indeces]
            ...
            save_sas_plot(c2g(np.abs(complex_bf), RP.circle_indeces, pix_dim, pix_dim),
