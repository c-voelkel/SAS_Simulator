import numpy as np
from sim_csas_package.utils_no_torch import interpfft
...
class Beamformer:
    def __init__(self, RP, **kwargs):
        ...
        assert self.interp in ['sinc', 'nearest'], "interp should be either sinc or nearest"
    ...
    def bf_trans(self, index):
        dist1 = np.sqrt(np.sum((self.pixels - self.trans[index].tx_pos[None].cpu()) ** 2, axis=1))
        dist2 = np.sqrt(np.sum((self.pixels - self.trans[index].rx_pos[None].cpu()) ** 2, axis=1))
        ...
        if self.interp == 'sinc':
            sinc_window = [np.sinc(tof - i) for i in range(0, self.RP.num_samples)]

            sinc_window = np.array(sinc_window)
            sinc_window = sinc_window.T
            sinc_window = sinc_window

            scene = np.sum(self.wfms[index, None] * sinc_window, axis=1)
        else:
            tof = tof.astype(int)
    ...
    def beamform_mp(self):
        ...
        # pool = mp.Pool(5)
        # # put each transducer on a different thread
        # scene_list = pool.map(self.bf_trans, range(len(self.trans)))
        #
        # # stack list and sum
        # scene_vector = np.stack(scene_list)
        # scene = np.sum(scene_vector, axis=0)
        # return scene

    def beamform_no_mp(self):

        scene = np.zeros(len(self.RP.circle_indeces))
        T_wfm = 1 / self.RP.Fs

        for i, trans in enumerate(self.trans):

            dist1 = np.sqrt(np.sum((self.pixels - trans.tx_pos[None].cpu()) ** 2, axis=1))
            dist2 = np.sqrt(np.sum((self.pixels - trans.rx_pos[None].cpu()) ** 2, axis=1))
            ...
            if self.interp == 'sinc':
                ...
                scene = scene + np.sum(self.wfms[i, None] * sinc_window, axis=1)
            else:
                tof = tof.astype(int)
