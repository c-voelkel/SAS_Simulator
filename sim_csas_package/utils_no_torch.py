...
def save_sas_plot(img, path, x_size=0.2, y_size=0.2, log=False):
    plt.clf()
    if log:
        plt.imshow(20 * np.log10(img), extent=[-x_size, x_size, -y_size, y_size], origin='lower')
...
def crop_psf(psf, thresh):
    ...
    cropped_psf = psf[SIZE // 2 - total_length:SIZE // 2 + total_length + 1, SIZE // 2 - total_length:SIZE // 2 + total_length + 1]
...
def TV(x):
    h, w = x.shape
    loss = np.sum(np.sum(np.abs(x[1:, :] - x[:-1, :])) + np.sum(np.abs(x[:, 1:] - x[:, :-1])))
    return loss

def L1_reg(x):
    h, w = x.shape
    loss = np.sum(np.abs(x))
    return loss
...
def grad_reg(x):
    ...
    kx = np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype=np.double)[None, None, ...]
    ky = np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype=np.double)[None, None, ...]

    eps = 1e-7

    x_d = cv2.filter2D(x, -1, kx[0, 0])
    y_d = cv2.filter2D(x, -1, ky[0, 0])

    xy_d = np.sqrt(x_d ** 2 + y_d ** 2 + eps).squeeze()
